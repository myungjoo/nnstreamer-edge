 ...
 * QUEUE_SIZE           | Max number of data in the queue, when sending edge data to other node. Default 0 means unlimited. N:<leaky [NEW, OLD]> where leaky 'OLD' drops old buffer (default NEW). (e.g., QUEUE_SIZE=5:OLD drops old buffer and pushes new data when queue size reaches 5.) nns_edge_get_info() returns both parts, e.g. 5:OLD or 0:NEW.
 ...
 *
 * @note QUEUE_SIZE does not bound the messages a MQTT or Hybrid node receives from the broker. Those are kept in a queue of their own that holds at most 100 and drops the oldest, and that bound cannot be changed.
 */
